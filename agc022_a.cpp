...
signed main(){
  ...
  if(S.size()<26){
    ...
    RREP(i,S.size()-2,-1){
      // REP(j,i,S.size()-1){
        int left=S.at(i)-'a',right=S.at(i+1)-'a';
        // print(left,"  ",right);
        if(left<right){
          longest=i+1;
          break;
    ...
    if(longest==0)print(-1);
    else{
      int app=-1;
      string origin=S.substr(0,longest);
      vector<int> chars(26);
      // chars[S.at(longest)-'a']++;
      // print(origin);
      each(c,origin){
        chars[c-'a']++;
      }
      REP(i,S.at(longest-1)-'a',26){
        if(chars[i]==0){
          char c=i+'a';
          S=S.substr(0,longest-1)+c;
          print(S);
          re 0;
        }
      }
