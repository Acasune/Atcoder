...
signed main(){
  string S;cin>>S;
  vector<int> schedule(4,0);
  REP(i,0,S.size()){
    if(S.at(i)=='N')schedule[0]=1;
    if(S.at(i)=='W')schedule[1]=1;
    if(S.at(i)=='S')schedule[2]=1;
    if(S.at(i)=='E')schedule[3]=1;
  }
  if((schedule[0]^schedule[2])||(schedule[1]^schedule[3]))print("No");
  else print("Yes");
